<template>
  <div class="px-6 py-6 space-y-6 pt-35 pb-24">
    ...
    <div class="flex gap-6">
      ...
      <div class="flex-1 space-y-6 min-w-0">
        ...
        <div class="grid md:grid-cols-2 gap-6">
          ...
          <div class="bg-[#1E1E1E] rounded-2xl p-6">
            ...
            <div class="flex justify-between items-center">
              ...
              <div>
                <span class="inline-block px-3 py-1 rounded-full bg-green-500/20 text-green-400 text-lg">
          ...
          <div class="bg-[#1E1E1E] rounded-2xl p-6 self-center">
            <div class="flex justify-between">
              <div>
                ...
                <p class="text-yellow-500 mb-3">Pionir Reksa Dana</p>
              </div>
              <div class="w-12">
                <img src="/archive-x.png" alt="Badge ETF" class="w-full">
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="bg-[#1E1E1E] rounded-2xl p-6 pb-20">
          ...
          <div id="tv_chart" class="w-full h-[400px] rounded-xl overflow-hidden border border-gray-700 bg-[#1A1A1A]"></div>
        </div>
          ...
          <div class="space-y-3">
            ...
            >
              ...
              <div class="text-right">
                ...
                <p :class="etf.change >= 0 ? 'text-green-400' : 'text-red-400'" class="text-sm font-semibold">
        ...
        <div class="bg-[#1E1E1E] rounded-2xl p-6">
          ...
          <div class="space-y-3">
            <div v-for="item in portfolio" :key="item.id" class="flex justify-between items-center p-3 rounded-xl bg-[#2C2C2C]">
              <div class="flex items-center">
                <div :class="`w-8 h-8 rounded-lg flex items-center justify-center text-white mr-3 ${item.color}`">
                  <i class="fa-solid fa-chart-line text-lg"></i>
                </div>
                <div>
                  <p class="font-medium">{{ item.name }}</p>
                  <p class="text-sm text-gray-400">Nilai: Rp {{ formatAmount(item.value) }} | {{ item.shares }} Lembar</p>
                </div>
              </div>
        ...
        <div class="bg-[#1E1E1E] rounded-2xl p-6">
          ...
          <div class="space-y-3">
            <div>
              ...
              <select v-model="selectedEtf" class="w-full bg-[#2C2C2C] border border-gray-600 rounded-lg px-4 py-2 appearance-none">
                <option value="" disabled selected hidden>Pilih ETF...</option>
                <option v-for="etf in etfs" :key="etf.symbol" :value="etf.symbol">{{ etf.name }} ({{ etf.code }})</option>
              </select>
            </div>

            <div>
              <label class="block text-sm text-gray-400 mb-1">Jumlah (Lembar)</label>
              <input type="number" v-model="investmentAmount" min="1" class="w-full bg-[#2C2C2C] border border-gray-600 rounded-lg px-4 py-2" placeholder="Contoh: 100">
            </div>
            ...
            <button 
              ...
              :class="[
                activeTab === 'beli' ? 'bg-green-600 hover:bg-green-700' : 'bg-red-600 hover:bg-red-700',
                (!selectedEtf || !investmentAmount || investmentAmount <= 0) ? 'opacity-50 cursor-not-allowed' : ''
              ]"
    ...
    <!-- Mobile Bottom Bar -->
    <div class="md:hidden fixed bottom-0 left-0 right-0 bg-[#1E1E1E] border-t border-gray-800 p-4 z-50">
      <div class="flex justify-between items-center max-w-4xl mx-auto">
        <div class="flex-1 mr-2">
          <select v-model="selectedEtf" class="w-full bg-[#2C2C2C] border border-gray-600 rounded-lg px-3 py-2 text-sm">
            <option v-for="etf in etfs" :key="etf.symbol" :value="etf.symbol">{{ etf.name }} ({{ etf.code }})</option>
          </select>
        </div>
        <button 
    ...
    <div v-if="showMobileModal" class="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-end md:hidden">
...
export default {
  ...
  methods: {
    ...
    executeTransaction() {
      ...
      this.$toast.success(
        `Berhasil ${action} ${this.investmentAmount} lembar ${etf.name}!`,
        {
          icon: 'success',
          position: 'top-right'
        }
